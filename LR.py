...
model = GridSearchCV(estimator=LogisticRegression(max_iter=3000), param_grid=params, scoring='accuracy', cv=5, n_jobs=1)
start = time.time()
model.fit(coo_train, class_arr)
print("Best parameters:{}".format(model.best_params_))  # 0.1
...
redis_conn.set('LR_pre_model', pickle.dumps(pre))
...
redis_conn.set('confusion_matrix_LR', pickle.dumps(C))
print("混淆矩阵为：\n", C)
# 计算准确率（accuracy）
...
df.to_csv("LR_classification_report.csv", index=True)
